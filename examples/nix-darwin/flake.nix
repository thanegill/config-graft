      ...
      darwinConfigurations.example = nix-darwin.lib.darwinSystem {
        modules = [
          config-graft.darwinModules.default
          {
            ...
            # (2) Reconciled through cfprefsd: set `cfprefsdDomain` so the merge goes
            # `defaults export` -> config-graft -> `defaults import`, instead of
            # editing the file (whose on-disk copy cfprefsd may override from cache).
            # Runs as root, so this targets the system/global domain.
