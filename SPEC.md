...
- **Scalars** — DESIRED replaces, *unless the two compare equal*: a value already
  equal to DESIRED's is left exactly as TARGET has it, so a difference in spelling
  alone (a JSON `2.5` against a `2.50`) is not a change and `--diff`, `--check` and
  the write all agree.
